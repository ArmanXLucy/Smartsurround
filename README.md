...
The dashboard now includes an integrated profile menu with:

- Update Profile: Firebase display name/email/password changes, phone preference, and profile avatar upload. Firebase Storage is used when enabled; a browser-local avatar fallback keeps the UI usable if Storage rules are not configured.
- Help Desk: complaint submission with Normal/Intermediate/Urgent priority, optional attachments, ticket IDs, and user ticket history/status/replies.
- Settings: light/dark/system appearance, notification preferences, and language preference persisted per account in local storage.
- Logout: confirmation dialog before Firebase sign-out and dashboard session cleanup.

The Help Desk uses the existing Flask backend. Set `VITE_BACKEND_URL` to the backend URL when the React app is deployed separately from Flask. The backend already allows the local Vite origins by default; production deployments should add their exact frontend origin to `CORS_ORIGINS`.

## Account Management / Help Desk

The profile menu now opens Update Profile, Help Desk, and Settings as full dashboard pages rather than floating modals. Logout uses the existing confirmation dialog.

For local development run the React app and Flask backend together:

```powershell
# Terminal 1
cd "H:\Smart Surround\SmartSurroundAI"
python app.py

# Terminal 2
cd "H:\Smart Surround\Smartsurround"
npm install
npm run dev
```

Vite proxies `/api`, `/admin`, `/uploads`, and `/letters` to `http://127.0.0.1:5000` during development, so Help Desk requests do not depend on browser CORS configuration. For a deployed frontend, set `VITE_BACKEND_URL` to the deployed SmartSurround backend URL.
